...
greyBlueStyle = " style=\"color:#85ACE1\""
greySmallStyleDark = " style=\"color:dimgrey;font-size:12px\""

lemsXmlUrl = "https://sourceforge.net/apps/trac/neuroml/browser/NeuroML2/NeuroML2CoreTypes/"
...
for file in files:
    lemscontents = lemsutil.readLems("../../../NeuroML2CoreTypes/%s.xml"%file, incIncludes=False)
...
for file in files:
    ...
    contents += "<div class=\"navbar \">\n"+ \
                ...
                "        <span class=\"icon-bar\"></span>\n"+ \
                "      </a>\n"+ \
                "      <a class=\"brand\" href=\"#\">NeuroML 2 Component Types</a>\n"+ \
    ...
    if lemscontents.getDescription() is not None:
        contents += ("    <table class=\"table table-bordered\"><tr><td ><h3>%s</h3></td></tr>\n"+ \
                    "    <tr><td>%s</td></tr>\n"+ \
                    "    <tr><td>Original LEMS definitions: <a href=\"%s%s.xml\">%s.xml</a></td></tr>\n"+ \
                    "    </table><br/>\n")%(file,formatDescription(lemscontents),lemsXmlUrl,file,file)
    ...
    if "Dimensions" in file:
        ...
        for unit in lemscontents.getUnit():
            ...
            contents += spacer4+"Dimension: "+dimension(unit.getDimension(), "", "")+"<br/>"+spacer4+"Power of 10: "+str(unit.getPowTen())+offset+"\n"

            for unit2 in lemscontents.getUnit():
                if unit.getSymbol() != unit2.getSymbol() and unit.getDimension() == unit2.getDimension():
                    diff = unit.getPowTen() - unit2.getPowTen()
    ...
    for compType in lemscontents.getComponentType():
        ...
        if len(compType.getComponentRef()) > 0:
            contents += "  <tr>\n"
            contents += category("Component References", len(compType.getComponentRef()), type="label-success")
            for cr in compType.getComponentRef():
        ...
        if len(compType.getBehavior()) > 0:
            for behavior in compType.getBehavior():
                contents += "<tr>\n"
                contents += category("Behavior", type="")
                contents += "<td colspan='2'>\n"


                if behavior.getBuild() is not None:
                    contents += "<span class=\"label\">Build</span><br/><br/>\n"
                    for w in behavior.getBuild().getWith():
                        contents += spacer4+"WITH <b>"+w.getInstance()+"</b> AS <b>"+w.getAs()+"</b><br/>\n"
                    if behavior.getBuild().getMultiInstantiate() is not None:
                        mi = behavior.getBuild().getMultiInstantiate()
                        contents += spacer4+"INSTANTIATE <b>"+mi.getNumber()+"</b> COMPONENTS OF TYPE <b>"+mi.getComponent()+"</b><br/>\n"
                    if behavior.getBuild().getEventConnection() is not None:
                        ec = behavior.getBuild().getEventConnection()
                ...
                if len(behavior.getOnEvent()) > 0:
                    contents += "<span class=\"label\">On Events</span><br/><br/>\n"
                    for oe in behavior.getOnEvent():
                ...
                if len(behavior.getDerivedVariable()) > 0:
                    contents += "<span class=\"label\">Derived Variables</span><br/><br/>\n"
                    for dv in behavior.getDerivedVariable():
                        res = dv.getValue()
                        if res is None:
                            res = dv.getSelect()
                    ...
                    if len(behavior.getDerivedVariable()) > 0: contents += "<br/>\n"
                

                if len(behavior.getTimeDerivative()) > 0:
                    contents += "<span class=\"label\">Time Derivatives</span><br/><br/>\n"
                    for td in behavior.getTimeDerivative():

                        contents += spacer4+"d <b>"+td.getVariable()+"</b> /dt = "+td.getValue()+"<br/>\n"
                    if len(behavior.getTimeDerivative()) > 0: contents += "<br/>\n"


                if len(behavior.getRegime()) > 0:
                    for rg in behavior.getRegime():
                    ...
                    if len(behavior.getRegime()) > 0: contents += "<br/>\n"
